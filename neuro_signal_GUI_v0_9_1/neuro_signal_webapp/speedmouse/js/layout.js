...
// This keeps the original SpeedMouse workbench intact while letting the local
...
function updateHeaderSummary(data) {
  ...
  const params = new URLSearchParams(window.location.search);
  const mode = params.get("comparison") ? "backend comparison" : (params.get("dataset") ? "backend dataset" : "offline toolbox");
  const datasetName = data?.meta?.dataset_id || data?.meta?.source_file || data?.meta?.backend_job_id || "dataset";
  headerStatus.innerHTML = `<span><strong>${channels}</strong> channels</span><span><strong>${frames}</strong> frames</span><span><strong>${mode}</strong></span><span><strong>${escapeHtml(String(datasetName).split(/[\/]/).pop())}</strong></span>`;
}

function datasetReadyLabel(data) {
  if (data?.meta?.loaded_from_backend || data?.meta?.backend_job_id || data?.meta?.loaded_dataset_url) {
...
function ensureBackendDatasetBanner(data) {
  if (!(data?.meta?.loaded_from_backend || data?.meta?.backend_job_id || data?.meta?.loaded_dataset_url)) return;
