...
function fallbackDatasetName(data) {
  const meta = data?.meta ?? {};
  if (meta.loaded_from_backend || meta.backend_job_id || meta.loaded_dataset_url) {
    const id = meta.dataset_id || meta.source_file || meta.source_recording_dir || meta.source_signal_path || meta.backend_job_id || "backend dataset";
    return `Neuro backend: ${String(id).split(/[\\/]/).pop()}`;
  }
  return meta.dataset_id || meta.source || "Demo dataset";
}
...
export function buildWorkbenchState({
  ...
  const datasets = activeSessions.length
    ...
    : fallbackData
      ? [{
        id: fallbackData?.meta?.backend_job_id || "default",
        name: fallbackDatasetName(fallbackData),
        data: fallbackData,
        active: true,
        isDefault: !fallbackData?.meta?.loaded_from_backend,
      }]
      : [];
